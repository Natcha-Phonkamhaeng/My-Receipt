...
# version 2.1 created 21 Feb 2022, adding str(text) for def separate_receipt
...
def separate_prefix(text):
	result = re.findall(r'BKKO|BKKL|BKKI|EPBKO|EPBKI|EPBKL|LCBO|LCBI|LCBL|SGZO|SGZI|SGZL\.?', str(text))
	return ' '.join(result)
...
app = App(root)
developer = Developer(root, 'V.2.1 Created by: Natcha Phonkamhaeng')
